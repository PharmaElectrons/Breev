...
Desktop diagnostics stay inside this boundary. React contains failures at the bootstrap, application, and active-workspace levels and shows localized recovery actions without rendering exception messages or stacks. Electron Main records only closed event records and privacy-safe incident fingerprints; ordinary logs rotate at 5 MiB with 20 files retained, the synchronous fatal sink is continuously capped, and the asynchronous queue drops excess breadcrumbs rather than delaying checkout input. Fatal notices use the asynchronous native message box, never the synchronous error box: that one spins a native nested loop Chromium never services with application tasks, so an unattended Main would freeze instead of exiting. Main exits only after the notice closes and pending breadcrumbs flush. Export builds a strict allowlisted JSON bundle, rejects any surviving synthetic canary, and commits through a same-directory staging file only after the native Main-owned Save dialog returns a destination. The renderer never receives or supplies a filesystem path.

Local export and the configured OS support handoff work offline and include no raw local-API, PostgreSQL, patient, prescription, transaction, credential, or installer-error payload. Central crash reporting is disabled by default. The `manual` configuration is only a gated integration mechanism: the authenticated UI exposes it only when Main reports that capability, every send requires a fresh operator confirmation, and failed/offline delivery does not affect local operation. Production activation, provider/region approval, retention, notices, and release evidence remain blocked by G-08, G-14, and G-16 in `open-decisions.md`.
...
Keep the desktop build Vite-compatible. Production builds use `electron-vite` for main, preload, and renderer and electron-builder's assisted per-machine NSIS target for `BreevSetup.exe`. The completed comparison keeps Electron Forge with MakerWix only as the issue-34 evidence harness; it adds no production dependency. The release proof still covers signing, Electron fuses, role-specific service and database lifecycle, repair, update, and recovery.

Development continues on the default `dev` branch. The protected `main` branch contains reviewed stable release candidates only. A release change keeps the root and desktop SemVer plus `CHANGELOG.md` aligned; an exact protected `vMAJOR.MINOR.PATCH` tag contained in `main` starts the Windows release workflow. During pre-production flow validation, that workflow creates an explicitly unsigned, draft GitHub prerelease and verifies the packaged version, payload lock, Electron fuses, hashes, provenance, and bounded evidence metadata without requiring secrets. It records `publishable: false`; Windows unknown-publisher warnings are expected, and the draft must not be promoted to a stable client release. Before production distribution, G-07 must select signing custody and the workflow must return to fail-closed signature and signer verification. A push to `main` alone never creates or publishes a release, and manual recovery may only rebuild an existing tag selected as the workflow ref. G-07 also retains the in-app update policy, staged-channel behavior, and final Windows evidence.
