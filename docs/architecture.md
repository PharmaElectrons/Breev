...
| Uninstall/reinstall | Removes application files, services, processes, and Breev firewall rules while preserving pharmacy data, configuration, and the pharmacy CA; reinstall reopens that state. | Removes application files while preserving role and pairing state; reinstall detects that preserved role without a new selection. |
