...
| `architecture.md`: the packager, updater, and installer choices stay open until the runtime-proof comparison completes (G-07) | Settled for the production path by the stakeholder decision of 29 August 2026, on the practical lifecycle proof rather than the certification ceremony. Production builds with `electron-vite` and electron-builder into the offline per-machine NSIS installer that runs the Breev install, repair, and uninstall lifecycle; Electron Forge with MakerWix stays a comparison harness under `tooling/windows/forge-comparison`. Signing identity, release host, and update policy remain milestone 4 in [`open-decisions.md`](open-decisions.md) G-07. |
