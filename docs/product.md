...
The supported roles are owner, manager, pharmacist, sales employee, purchasing employee, inventory employee, accountant, and support, each with configurable permissions for screens, actions, cost prices, exports, edits, reports, and sensitive transactions. Account permissions, not device type, determine what an employee can do from any licensed device. Breev Support and cloud operators are exceptional actors who require separate authorization; they are never ordinary pharmacy users.
