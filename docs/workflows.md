...
1. Open the server-provided, pharmacy-scoped draft list with `sales.drafts.manage`; an authorized user can explicitly open or resume any draft in that pharmacy, regardless of its creator or last device, from a device that passes authentication and device-trust checks. Creator and device remain audit metadata. Automatic selection when Sales opens remains unresolved (see `open-decisions.md`). Invoices can be suspended and resumed without losing data, and unsaved invoice data survives navigating away and back. Clicking **New**, or deleting or clearing an invoice that contains items, requires confirmation before any data is lost.
...
1. Open the Count Sessions screen from Inventory. Resume a listed active session; the current UI disables starting another while an active session is listed. An older active session remains open for its unfinished count or pending variance and must be completed explicitly rather than silently discarded. Completed sessions appear separately and remain reviewable. The permitted concurrency policy across users/devices is still open in `open-decisions.md`.
...
The operational Inventory grid is read-only and uses the full available width. Selecting a row gives a visible state and a route to the approved Item Movement Details; no inventory sidebar or branch-transfer control is part of Phase One. Four summary cards show total inventory value, average unit cost, distinct items, and items with stock; valuation cards show no amount when valuation permission is denied. The grid has a live search input, sortable approved columns with explicit sort direction, column settings, an Order Basket link, protected export, and Count Sessions. Retail-price sorting is not part of this inventory view. The old prototype's separate Search, Refresh, Print, and Exit toolbar controls are not part of this view: search is already live, reload follows normal data refresh, inventory printing is not approved here, and child pages provide a return route.
