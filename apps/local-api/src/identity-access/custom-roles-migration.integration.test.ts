  ...
  it("preserves identities while granting manager and owner permissions once", async () => {
    ...
    for (const before of rolesBefore) {
      ...
      expect(after?.revision, before.role_key ?? before.id).toBe(
        before.role_key === "owner"
          ? String(BigInt(before.revision) + 9n)
          : before.role_key === "manager"
            ? String(BigInt(before.revision) + 6n)
            : before.revision,
      );
    }
    ...
    expect(await snapshotGrants()).toEqual(
      [
        ...
        {
          granted_by: ownerId,
          permission_name: "inventory.reorder.confirm",
          role_id: managerRoleId,
        },
        {
          granted_by: ownerId,
          permission_name: "inventory.reorder.manage",
          role_id: managerRoleId,
        },
        {
          granted_by: ownerId,
    ...
    expect(await pharmacyRevision()).toBe(String(BigInt(revisionBefore) + 10n));
