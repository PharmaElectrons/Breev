...
describe.sequential("purchasing role default migrations", () => {
  ...
  it("preserves a customized role and advances an untouched old default once", async () => {
    ...
    // 0018, 0019, 0020, 0021, 0022, 0023, and 0024 each advance the pharmacy identity revision
    ...
    expect(await revisions()).toEqual({ pharmacy: "8", role: "1" });
    ...
    expect(await revisions()).toEqual({ pharmacy: "9", role: "2" });
    ...
    expect(await revisions()).toEqual({ pharmacy: "10", role: "3" });
