...
export function observeTrustedTime(input: {
  ...
  if (input.inMemoryHighWater !== undefined) assertDate(input.inMemoryHighWater);
  if (input.persistedLowerBound !== undefined) assertDate(input.persistedLowerBound);

  const previous = maxDate(
    input.inMemoryHighWater,
    input.persistedLowerBound,
  );
  ...
  const shouldPersist =
    persistedAt === undefined || now.getTime() - persistedAt >= PERSISTENCE_CADENCE_MS;
  return {
...
function maxDate(left?: Date, right?: Date): Date | undefined {
  if (left === undefined) return right === undefined ? undefined : new Date(right);
...
function assertDate(value: Date): void {
  if (!Number.isFinite(value.getTime())) throw new TypeError("Trusted time must be valid");
}
