...
describe("Manual Interactive Durable Jobs Demonstration", () => {
  it("runs end-to-end interactive manual verification of pg-boss durable jobs", async () => {
    ...
    console.log(
      "       -> Database ready with breev_schema_owner and breev_app roles.",
    );
    ...
    console.log(
      "       -> Verified pgboss tables created: ",
      schemaCheck.rows.map((r) => r.table_name).join(", "),
    );
    ...
    await durableJobs.work(
      "order-receipt-queue",
      async (job: { data: { orderId: string } }) => {
        processedMessage = `Order ${job.data.orderId} receipt created!`;
        console.log(
          `       [Worker Output]: Processed job -> "${processedMessage}"`,
        );
      },
    );

    await db.transaction(async (tx) => {
      await durableJobs.sendInTransaction(tx, "order-receipt-queue", {
        orderId: "ORD-9981",
      });
      console.log(
        "       -> Job queued inside Drizzle transaction. Committing transaction...",
      );
    });
    ...
    console.log(
      "       -> Result: Job successfully picked up by worker after commit.",
    );
    ...
    try {
      await db.transaction(async (tx) => {
        await durableJobs.sendInTransaction(tx, "rollback-test-queue", {
          orderId: "ORD-FAIL",
        });
        console.log(
          "       -> Job queued inside transaction. Simulating business failure & rolling back...",
        );
    ...
    console.log(
      "       -> Result: Zero jobs were processed. Rollback atomicity verified!",
    );
    ...
    let attemptCount = 0;
    await durableJobs.work("failing-queue", async () => {
      attemptCount += 1;
    ...
    console.log(
      "       -> Dead letter state:",
      JSON.stringify(deadLetterJobs[0], null, 2),
    );
    expect(deadLetterJobs.length).toBeGreaterThanOrEqual(1);
    expect(deadLetterJobs[0]?.state).toBe("failed");
    console.log(
      "       -> Result: Job retried and moved to dead-letter state successfully.",
    );
